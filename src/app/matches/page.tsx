    ...
    <div className="min-h-screen bg-black cyber-grid">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="space-y-4">
              {futureUpcomingMatches.length === 0 ? (
                <div className="bg-gray-900/50 rounded-xl border border-yellow-400/20 p-8 text-center">
                  ...
                  <p className="text-gray-400 text-sm mt-2">新しい試合が決まり次第、こちらに掲載します。</p>
                </div>
              ) : (
                  ...
                  return (
                    <div key={match.id} className={`bg-gray-900/50 rounded-xl border ${resultStyle.border} p-6 hover:border-opacity-60 transition-all duration-300`}>
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className={`text-3xl font-bold mb-1 ${resultStyle.text}`}>{`${match.homeScore} - ${match.awayScore}`}</div>
        ...
        <div className="flex justify-center mt-12 pb-8">
          <Link href="/" className="inline-flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-yellow-400 to-yellow-600 text-black font-semibold rounded-lg hover:from-yellow-300 hover:to-yellow-500 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105">
            <ArrowLeft size={20} />
            <span>HOME</span>
          </Link>
        </div>
